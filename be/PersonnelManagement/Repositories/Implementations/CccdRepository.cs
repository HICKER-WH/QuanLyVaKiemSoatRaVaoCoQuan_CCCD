...
namespace PersonnelManagement.Repositories.Implementations
{
    public class CccdRepository : ICccdRepository
    {
		...
		public async Task<PersonnelHistory> ProcessGuestCheckInOut(string numberId)
		{
			// Tìm bản ghi lịch sử gần nhất của khách dựa trên Note
			var latestHistory = await _context.PersonnelHistories
					.Where(h => h.PersonnelId == null && h.Note.Contains($"NumberId: {numberId}"))
					.OrderByDescending(h => h.Timestamp)
					.FirstOrDefaultAsync();

			// Xác định trạng thái mới
			var newStatus = (latestHistory?.Status == Status.CheckIn) ? Status.CheckOut : Status.CheckIn;

			// Tạo bản ghi lịch sử mới
			var history = new PersonnelHistory
			{
				Id = Guid.NewGuid(),
				PersonnelId = null, // Để null vì không có trong Personnels
				Status = newStatus,
				Timestamp = DateTime.UtcNow.AddHours(7),
				Note = $"Automated {newStatus} for Guest with NumberId: {numberId}"
			};
		...
		public async Task<Personnel?> ProcessCheckInOut(string numberId)
        {
            ...
            if (personnel == null)
            {
				        var guestHistory = await ProcessGuestCheckInOut(numberId);
				        return null;
            }

...
namespace PersonnelManagement.Controllers
{
    ...
    public class PersonnelController : ControllerBase
    {
        ...
        private readonly ILogger<PersonnelController> _logger;
		    ...
		    public PersonnelController(ICccdRepository cccdRepository, ILogger<PersonnelController> logger, ApplicationDbContext context)
		    {
            ...
            try
            {
                if (string.IsNullOrEmpty(numberId))
                {
                    return BadRequest(new { Message = "Number ID is required" });
				        ...
				        if (!Regex.IsMatch(numberId, @"^\d{12}$"))
				        {
					        return BadRequest(new { Message = "NumberId is invalid." });
				        }

				        var result = await _cccdRepository.ProcessCheckInOut(numberId);
				        if (result == null)
				        {
					        var latestGuestHistory = await _context.PersonnelHistories
							        .Where(h => h.PersonnelId == null && h.Note.Contains($"NumberId: {numberId}"))
							        .OrderByDescending(h => h.Timestamp)
							        .FirstOrDefaultAsync();

					        //string statusMessage = latestGuestHistory?.Status == Status.CheckIn ? "Check-in" : "Check-out";
					        return NotFound(new
					        {
						        Message = "Personnel not found, recorded as Guest",
						        Data = new
						        {
							        NumberId = numberId,
							        Status = latestGuestHistory?.Status ?? Status.CheckIn,
							        Timestamp = latestGuestHistory?.Timestamp ?? DateTime.UtcNow.AddHours(7)
						        }
					        });
				        }
